...
- 予約者はマイページから自分の予約をキャンセルできます(枠の空きはすぐ解放されます)。キャンセルは予約記録の取り消しのみで、Stripeでの自動返金は行いません(返金が必要な場合は管理者がStripe管理画面から個別に対応してください)。キャンセル後もそのレッスンの開催日までは「キャンセル済み」としてマイページに表示され続けます
...
## サイトの見栄え(サムネイル・講師紹介・口コミ)

- 動画講座・レッスンの管理画面(新規追加・編集)に「サムネイル画像URL」欄を追加。画像URLを入力するとその画像がサムネイルとして表示され、空欄の場合は従来通りグラデーション+再生アイコンを表示します(アップロード機能はなく、既存の動画URL欄と同様、画像を外部にホスティングしてURLを貼り付ける形式です)
- `/admin/settings` に「講師プロフィール」欄を追加。お名前・写真URL・自己紹介文を入力すると、トップページに講師紹介セクションが表示されます(お名前が空欄の間は非表示のまま)。写真URL未設定の場合は名前の頭文字を使ったアバターを表示します
- `/admin/testimonials` で口コミ(生徒さんの声)を管理できます。お名前・コメント・写真URL(任意)・評価(任意、1〜5の★)・公開/非公開を設定でき、公開中の口コミがある場合のみトップページに「生徒さんの声」セクションが表示されます
